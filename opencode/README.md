...
| well (init) | `openrouter/moonshotai/kimi-k3` | bumped 2026-08-09 from `gemini-2.5-flash` — runs once per project, so cost barely matters, but the docs it writes are foundational to everything downstream. 1M-token context covers all five gated docs at once without the compaction risk a smaller window would carry |
