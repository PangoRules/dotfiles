...
| `agents/erwin.md` | **Commander (orchestrator)** — branch setup → dev → review loop → docs → docs recheck → E2E gate → PR. Anything that isn't a plan-execution request (bug report, new idea, question) is classified via the `roster-routing` skill and dispatched directly — no bounce through `@gandalf` for a call it can make itself. A dispatch to `@mikasa` this way isn't done when the fix lands — `post-fix-review` (levi, independently) has to LGTM it first |
...
| `skills/lint-format-detect` | Detects the actual configured linter/formatter per language (e.g. CSharpier via a project's tool manifest, not an assumed default) instead of guessing one tool per language — used by both `@levi` (review) and `@arthur` (pre-commit) |
| `skills/model-preflight` | Verifies ollama is reachable, models are pulled, and context length isn't silently truncating prompts — run once per session and as a diagnostic before assuming a local-model agent is dead |
| `skills/agent-config-lint` | Manual maintenance check for whoever edits `agents/*.md`/`opencode.json` — dangling `@mentions`, dangling skill references, prose hard-rules not backed by a matching permission block, reserved-key collisions. Not part of any runtime dispatch |
| `skills/accessibility-audit` | Detects a project's own stated a11y requirement + tooling (axe/eslint-a11y plugins), falls back to a manual keyboard/labeling/contrast checklist. No-ops if the project states no requirement |
| `skills/api-contract-diff` | Diffs a generated API schema (OpenAPI/GraphQL SDL/protobuf) before vs after — catches breaking changes a same-repo build won't, since a decoupled client (e.g. one with no `ProjectReference` to the server) never fails to compile even when the contract breaks under it |
| `skills/frontend-bundle-budget` | Builds both branch tips, flags a bundle-size regression past 10% on diffs that aren't themselves a new feature route |
| `skills/pipeline-status` | Answers "where am I / what's the status" from disk + git state without requiring the human to know which primary agent's own resume logic to invoke — `@gandalf`'s home for this via `roster-routing` |
| `skills/unit-test-convention-detect` | Detects a project's existing unit test naming/AAA/fixture/mock conventions before `@arthur`/`@tarnished` write a new test — same detect-first shape as `docs-convention-detect`, stack-agnostic |
| `skills/http-file-convention-detect` | Detects a project's existing `.http` file location/auth-chaining/ordering conventions before `@arthur`/`@tarnished` create one — authoring half of what `@levi`'s `http`-scope check already verifies exists |
| `skills/e2e-test-convention-detect` | Detects a project's existing e2e selector/wait/structure conventions before `@arthur`/`@tarnished` write a new spec; hard-codes condition-based waits over fixed sleeps, grounded in a real regression this codebase already hit |
| `skills/secret-leak-scan` | Cheap grep-pattern check for hardcoded credentials in a diff's added lines — runs on every `@levi` review regardless of whether `@carter` was separately invoked, the gap that let a hardcoded key ship clean without one |
...
### 2026-08-09 pass — infra-blocked review loops, lint tool assumptions, roster drift

Found via a real session: a downed local service (MinIO not running) made `@levi`'s test run fail with a connection error, which the old rules treated as a plain code finding — `@arthur` had nothing to fix, the same finding survived the next cycle, and the loop burned cycles before `review-cycle-diff`'s stuck-detection would have caught it. Separately, the old lint/format check listed `.editorconfig` + `dotnet format` as *the* .NET tool, missing that a project might use CSharpier instead (this dotfiles repo's own `hydra-forge` project does — CI runs `dotnet csharpier check .`, not `dotnet format`).

Fixed:
1. **`@levi` now runs `docker-preflight` before any test suite**, not just when arthur happens to touch DB/storage code. A test failure whose own error text is an infra signature (connection refused, no-such-host, a docker-compose port timeout) that `docker-preflight` couldn't resolve is reported as `INFRA-BLOCKED` — a literal, greppable signal (same treatment as `LGTM`) — instead of a normal finding. `@erwin` now branches on it: stops, tells the user to start the missing service, and does **not** count the cycle or dispatch `@arthur` to "fix" infrastructure that isn't running.
2. **Lint/format detection extracted into its own skill** (`lint-format-detect`) with a real per-signal-file table instead of an inline guess-list — CSharpier (via `.config/dotnet-tools.json` or `.csharpierrc`) is detected and preferred over bare `dotnet format` when both could apply. `@arthur` now runs it before every commit too, not just `@levi` at review time — catches format issues before they cost a review cycle.
3. **`model-preflight` skill** — the `OLLAMA_CONTEXT_LENGTH` truncation incident (below) was diagnosed by hand once; this makes the check repeatable. `@fire_keeper` runs it once per session; `@erwin` suggests it via `@tarnished` before reporting a local-model agent dead, since silent truncation looks exactly like a dead agent.
4. **`agent-config-lint` skill** — every entry in this file's own "Known incidents" (in `agents/erwin.md`) was roster-config drift a human caught mid-session. This is the check that would have caught several of them at edit time instead: dangling `@mentions`, dangling skill references, a hard "never do X" prose rule with no matching `opencode.json` permission block, a new agent file colliding with an opencode-reserved key. Manual, run by whoever edits the roster — not wired into any runtime dispatch.
5. **Coverage additions to `@levi`'s stack-specific list**: `accessibility-audit` (a11y, gated on the project actually stating a requirement), `api-contract-diff` (breaking-change detection for generated API clients — written for the exact shape where a TUI/mobile client with no compile-time link to the server can't catch a broken contract by building), `frontend-bundle-budget` (size-regression check on dependency/route changes). Plus `pipeline-status` for "where am I" questions, and a size-advisory addition to `plan-shape-check` (flags an oversized plan without failing it) and a license-compliance step added to `dependency-vulnerability-scan`.

All of the above are stack-detection-based, same pattern as the existing skills (`error-handling-consistency-check`, `docs-convention-detect`) — they check what a project actually has configured rather than assuming a language or a specific project's conventions, so none of it is tied to any one project.
...
Mixed local/cloud since 2026-08-08. Strategy: MiniMax subscription is spent ONLY where model quality changes real outcomes (reviewer quality gate) or where a local model's own request profile doesn't fit the hardware reliably. Deterministic/gate-keeping roles (`git`, `fire_keeper`, `tarnished`) moved to local ollama on the RTX 4090 rig — they were burning MiniMax subscription quota for work that doesn't need a frontier-tier model, and the shared pool (5 agents deep before this change) was the real driver of hitting caps. Implementation (`developer`) moved local the same day, then back to MiniMax on 2026-08-09 — its prompts (full file reads, multi-finding fix batches) turned out to be the heaviest and least predictable in the roster, and this card doesn't have headroom to spare for that at the context sizes those prompts need. `architect`/`carter`/`strelok` stay on cheap-to-mid OpenRouter — rare on-demand reasoning, neither contributes to the MiniMax cap problem, no reason to move them.

**2026-08-09 update:** `docs` (iroh) moved to local ollama as a test — deterministic/low-judgment profile, same shape that already succeeded locally for fire_keeper/hosea/tarnished. `brainstorm` (armin) moved the other direction, up to `deepseek-v4-pro` — bumped to match sokka/levi's tier since all three reasoning-heavy roles (brainstorm/architect/review) should sit equally strong, not two-of-three. `well` (the_well) was tried on local ollama in the same pass, then reverted same-day: it runs once per project, so the local cost-saving rationale barely applies (call volume is naturally tiny regardless of tier), while the stakes are high — five foundational docs everything else builds on. Moved instead to `moonshotai/kimi-k3` (2.8T params, 1M-token context, verified live on OpenRouter as of 2026-08-09) — its headline strength, large context, is exactly what synthesizing scope/architecture/data-model/glossary/spec into one coherent set actually needs. **Confirmed by hands-on use, not just spec:** `kimi-k3` has been the most reliable model tested so far for actually drafting docs — `glm-5.2` was evaluated as a candidate the same day and found finicky for this specific job, so `well` stays on `kimi-k3` regardless of any future pricing comparison.
...
| **developer** (arthur) | `minimax-coding-plan/MiniMax-M2.7` | **settled on cloud, 2026-08-09, after two local attempts.** `qwen3-coder:latest` was fine on paper but slow in practice — a heavy dispatch (15 reviewer findings + several files read for context) exceeded what fits in VRAM at `OLLAMA_CONTEXT_LENGTH=98304` alongside other GPU load, ollama fell back to 86% CPU inference, and the call crawled for minutes. Tried `devstral-small-2:latest` next (smaller, more VRAM headroom) — it made real progress (more than qwen3-coder had) but still died mid-cycle repeatedly, even after bumping `limit.output` 8192→16384 and `agent.arthur.steps` 30→60. Root cause wasn't a ceiling to keep raising: devstral's own working style on this roster's heaviest task shape (multi-finding fix batches, lots of re-read-to-verify between edits) burns tool-call/output budget on caution rather than progress, and every failed restart re-pays a "figure out what's already fixed" tax with no memory of the prior attempt — a convergence problem, not a config one. Confirmed by contrast: the same fix, once re-scoped to `@tarnished`'s lighter no-ceremony process, finished cleanly even on the *smaller* local `glm-4.7-flash`. Conclusion: `developer`'s prompt shape (full file reads, iterative multi-finding fixes) is too token/step-heavy to delegate locally in this roster — settled back on MiniMax-M2.7, its pre-local model, on purpose accepting the cost. `git`/`fire_keeper`/`tarnished`/`iroh` stay local — none of them carry this same heavy, iterative-fix-cycle shape. |
...
| well (init) | `openrouter/moonshotai/kimi-k3` | bumped 2026-08-09 from `gemini-2.5-flash` — runs once per project, so cost barely matters, but the docs it writes are foundational to everything downstream. 1M-token context covers all five gated docs at once without the compaction risk a smaller window would carry. Re-confirmed same day against `glm-5.2` as a candidate — kimi-k3 held up as the more reliable doc-drafting model in hands-on testing, glm-5.2 was finicky for this job specifically |
...
> **Historical note:** `qwen3-coder:latest` was the original local developer model, dropped for cloud-only, briefly readded as an OpenRouter-hosted fallback, then reinstated as the local `developer` default on 2026-08-08 after Devstral (tried first in the same pass) hit the `OLLAMA_CONTEXT_LENGTH` incident documented under "Local model ops" below — `glm-4.7-flash` and `gpt-oss:20b` were also pulled in that pass and are available to swap in (see "Registered but not wired" below the table). Dropped again, this time for MiniMax-M2.7, on 2026-08-09 (see the table above) — not a repeat of the context-length incident (that was fixed, and `journalctl` confirmed no truncation this time), just `developer`'s prompts being consistently the heaviest in this roster and this card not having room to spare once other GPU load is present. `openai/gpt-5.5` was a previous reasoning-tier model no longer used. docs and git were both found drifted from their documented models/temps during a July 2026 audit — the pre-2026-08-08 table reflected that corrected state, since superseded by the local-model move above. Primary agents were renamed (init→well, planner→fire_keeper, orchestrator→commander, builder→tarnished) plus security→carter, explore→Strelok, debugger→Mikasa, and router→Gandalf added across two later passes — see "Agent names" at the top.

**Candidates evaluated but not adopted (2026-08-09):** `openrouter/z-ai/glm-5.2` — priced at $0.07/$0.22 per 1M tokens on OpenRouter (explicit "95% off" promotional flag on that listing, not a standing rate), 1M context, SOTA-tier open-weights benchmark reputation. Looked like a strong candidate to consolidate most of the non-MiniMax roster onto for cost, but this was analysis only — nothing was meant to be implemented from it yet, and hands-on testing the same day found it finicky specifically for doc-drafting work (`well`'s job), where `kimi-k3` has been the more consistently reliable model in actual use. Worth keeping in mind if it's re-evaluated later, but not adopted anywhere in the table above.

### Fallback models (manual) — full roster, 2026-08-09

opencode has no native model-fallback field (`AgentConfig.model` is a single string — confirmed against `https://opencode.ai/config.json` schema, and against `packages/opencode/src/agent/agent.ts` directly). Flip the affected agent's `model:` line by hand for whichever failure mode below applies. No auto-switching exists — see the community-plugin note at the bottom for why one isn't installed.

Two failure modes cover the whole roster: the **local rig is unreachable** (machine off, asleep, away from it), or **MiniMax subscription quota/infra is down**. Only 4 agents are exposed to the first mode — everything else already runs on OpenRouter/MiniMax and needs no action when the local machine is unavailable.

**Local rig unreachable** — only agents that currently run on `ollama/*`:

| Agent | Primary (local) | Fallback | Why this fallback, not MiniMax |
|-------|------------------|----------|-------------------------------|
| hosea (git) | `ollama/glm-4.7-flash:latest` | `openrouter/deepseek/deepseek-v4-flash` | Deterministic git/PR mechanics, zero real judgment calls — flash-tier is plenty, and this is exactly the load you don't want competing with `arthur`/`levi`/`mikasa` for MiniMax quota while unattended |
| fire_keeper (planner) | `ollama/glm-4.7-flash:latest` | `openrouter/deepseek/deepseek-v4-flash` | Pure gate-keeping/relay — same reasoning as git |
| tarnished (builder) | `ollama/glm-4.7-flash:latest` | `openrouter/deepseek/deepseek-v4-flash`, escalate to `openrouter/minimax/minimax-m2.7` only if a specific small-task dispatch is visibly struggling | Does real (small-scoped) implementation, not pure mechanics, so it's the one row worth a step-up option if flash-tier proves too weak for a given task |
| iroh (docs) | `ollama/glm-4.7-flash:latest` | `openrouter/deepseek/deepseek-v4-flash` | Its own documented pre-local model (see "2026-08-09 update" above) — already proven, not a new guess |

Rationale for picking flash-tier OpenRouter over MiniMax across the board here: MiniMax is a flat-fee subscription with a real quota ceiling, currently spent on the 3 roles where quality most directly drives code/plan consistency (`arthur`, `levi`, `mikasa`). Being away from the machine means nobody's watching quota — the worst time to have 4 more agents competing for it. Flash-tier OpenRouter is cheap enough that daily heavy use across all 4 stays well inside a $20-30/mo ceiling (low per-token cost × deterministic, usually-short prompts), while keeping MiniMax's headroom fully reserved for the quality-critical trio.

**Already cloud-resident — no fallback needed for this failure mode, unaffected by the local rig being unreachable:** `arthur` (developer, `minimax-coding-plan/MiniMax-M2.7`), `levi` (reviewer, `minimax-coding-plan/MiniMax-M3`), `mikasa` (debugger, `minimax-coding-plan/MiniMax-M2.7`), `sokka` (architect), `armin` (brainstorm), `well` (init), `carter` (security), `strelok` (explore) — all `openrouter/deepseek/*` or `openrouter/moonshotai/*`, `erwin` (commander), `gandalf` (router) — both `openrouter/deepseek/deepseek-v4-flash`.

**MiniMax subscription quota exhausted or MiniMax infra down** (affects `arthur`, `levi`, `mikasa` — the 3 agents that actually depend on it day-to-day as of 2026-08-09):
...
Revert to `minimax-coding-plan/MiniMax-M*` once quota/infra recovers — tiers above are paid-per-token, not subscription. Revert local agents to `ollama/*` once the machine/GPU is available again — cloud fallback costs real money per call, local is free once pulled.

**Automatic fallback exists via community plugin, not installed here:** `opencode-fallback` (youngbinkim0/opencode-fallback) and `opencode-rate-limit-fallback` (liamvinberg/opencode-rate-limit-fallback) both add chain-on-failure switching since opencode core doesn't. Not added — third-party code with full session access is worth reading before trusting. Worth revisiting if manually flipping 4 agent files before leaving the machine becomes a recurring chore rather than a rare one.
...
**`context-mode` enforcement is global, not per-agent (2026-08-09):** routing (blocking raw `curl`/`cat`/large `grep` in favor of `ctx_*` sandbox tools) is done by the plugin's own `tool.execute.before`/`tool.execute.after` hooks — this fires for every agent and subagent the moment the plugin is loaded (`opencode.json`'s top-level `plugin` array, above the per-agent `agent` block), same as the secrets-protection `permission` block. There is no `AgentConfig` field to opt an individual agent in or out. The one missing piece was model *awareness*: without a routing-rules file, a blocked command just fails with no explanation and an agent can burn a retry re-attempting it blind. Root `AGENTS.md` (copied from `context-mode`'s own `configs/opencode/AGENTS.md`) fixes that — OpenCode's `experimental.chat.system.transform` hook (the plugin's SessionStart surrogate, see its README) injects it into every session's system prompt at start, primary and subagent alike. Verify with `ctx doctor` inside any session.

Every `agents/*.md` file also carries its own explicit `MANDATORY: Follow this project's root AGENTS.md context-mode routing rules` line, right under its `caveman` line — belt-and-suspenders on top of the hook/`AGENTS.md` injection above, same pattern this file already uses for `caveman`. Not required for enforcement (the hook doesn't care whether the persona file mentions it), but keeps it visible in-file rather than relying entirely on an injected system-prompt block nobody editing this roster will ever see.
...
**Critical fix (2026-08-09) — the deny rules above were silently inert until this pass.** Confirmed against opencode's actual source (`packages/opencode/src/permission/index.ts` on `sst/opencode`, `dev` branch): permission resolution flattens every applicable ruleset into one array and resolves a request with `.findLast(...)` — **last matching rule in the array wins** — and the config parser explicitly preserves JSON key order for this exact purpose (`propertyOrder: "original"`, "so user key order is preserved for permission precedence," per `packages/core/src/v1/config/permission.ts`). Every block above (`read`/`edit`/`grep`/`bash`) had its `"*": "allow"` catch-all written as the *last* key, after every specific deny rule — meaning that catch-all, not the denies above it, was the winning match for every single request, since `"*"` compiles to a match-anything regex (`^.*$`) that also matches `.env`, `credentials.json`, etc. The denylist has never actually blocked anything since this section was written. Fixed by moving `"*": "allow"` to the *first* key in each block, so every specific deny listed after it now wins per the same last-match-wins rule. Lesson for editing this file going forward: **in this permission schema, specificity does not win — position does. The override always goes last.**

**Also added (2026-08-09) — `external_directory`, `glob`, `list`, `webfetch`, `websearch`, `skill` were never in this block at all.** opencode's own built-in per-agent defaults already allow almost everything by default, *except* `external_directory`, which defaults to `ask` for any path outside the current worktree and a narrow built-in whitelist (opencode's own tmp dir, skill dirs) — system `/tmp` and sibling project folders (e.g. `~/Projects/*`) were never covered, so any subagent touching either triggered a fresh prompt every time, with no way to pre-approve it project-wide. This is what was actually behind the "stuck asking for permission in the project folder and in tmp" complaint that started this pass — not an inheritance gap (subagents already inherit this top-level block fine, confirmed against the same source: `Permission.merge(defaults, user)` in `packages/opencode/src/agent/agent.ts`). Added `external_directory: {"*": "ask", "/tmp/*": "allow", "~/Projects/*": "allow"}` plus blanket `allow` for `glob`/`list`/`webfetch`/`websearch`/`skill`. Scoped this loosely on purpose: every agent in this roster only ever operates inside a branch `@hosea` already isolated before handing off (see Step 2 of `@erwin`'s loop), so the remaining real risk is destructive git ops, not stray file reads — those stay separately guarded by this repo's own git-safety norms (no `--force`, no `reset --hard`, no `--no-verify`), not by opencode's permission block.
