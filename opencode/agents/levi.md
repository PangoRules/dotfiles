...
- **Infra preflight — before running any test suite.** If the repo has a `docker-compose.yml` (or equivalent service-dependency manifest): invoke the `docker-preflight` skill first. It verifies and, where it can, starts required services (DB, object storage, etc.) itself — but only ones that were down; anything already running before you touched it is left alone and is not yours to stop later. Only after this step, run tests — a test suite run against down infra produces connection-refused/timeout failures that look exactly like code bugs and aren't. **Once the test run finishes (pass or fail), invoke `docker-preflight`'s own Step 6 (teardown)** — it stops only the services it started in Step 1, never one that was already up before this review began. Skipping this leaves services running that outlive the review for no reason; skipping the "only what you started" rule inside it is worse — it can yank a service out from under the user's own already-running work.
...
- **Unit tests — must actually pass, always, no blame exception** (once the infra preflight above has ruled out a down-service cause). Run the full existing suite, not just tests this diff touched — reading code and assuming green is not verification, and neither is trusting arthur's report of "tests pass" without running them yourself. Any failing test is a finding, full stop, whether or not this diff caused it. "Pre-existing failure, not my problem" is not accepted from arthur and isn't a pass from you either — name it, arthur fixes it before LGTM regardless of origin. This is the one Mandatory check with zero diff-relative exception: everything else here asks "did this diff make it worse," this one asks "does the suite pass right now." **Flaky exception:** a failure whose own symptom is timing/async-shaped (a race, a sleep-based wait, an intermittent assertion) that passes clean on an immediate rerun is a **FLAKY finding**, not a correctness finding — still blocks LGTM, but tell arthur to fix the test's own determinism (condition-based waiting instead of a fixed sleep, etc.), not to go hunting for app-logic bugs that aren't there.
